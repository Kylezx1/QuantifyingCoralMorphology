...
#vcgPlyRead ====
#vcgPlyRead reads in ply files and is a function in the "geomorph" package.
library(geomorph)

#vcgPlyRead() this is the function

#plyFilePrep ====
#This is a function I previously used to read in PLY files before I discovered geomorph.
#should be deprecated now, but leaving here in case.
plyFilePrep <- function(PlyFile) { #function that converts an ASCII .ply file into a format suitable for surface area and volume calculations by extracting the coordinates and vertex indices
  VertexGet <- fread(PlyFile,skip = 1, nrow = 1, sep = "\n") #extract the line containing the total number of vertices in the ply file
  VertexCount <- as.numeric(paste((unlist(regmatches(VertexGet, gregexpr("[0-9]", VertexGet)))), sep= "", collapse ="")) #grep the number of verticies from VertexGet
  XYZDat <- as.matrix(fread(PlyFile, skip = 9, nrow = VertexCount)) #A matrix containing all XYZ coordinates in the PLY file
  VertDat <- as.matrix(fread(PlyFile, skip = 9+VertexCount)) #A matrix containing all vertex pointers for each mesh triangle's 3 XYZ coordinates
  VertDat <- VertDat[,2:4] #strip out the column of "3"'s from the martix
  VertDat <- VertDat + 1 #Add a constant of 1 to make the smallest vertex pointer non-zero
  VertDat <- as.vector(t(VertDat)) #transpose the vertex pointer matrix and convert to a vector
  MeshDat <- XYZDat[VertDat,] #extract the XYZ coordinates for each vertex pointer
  MeshDat <- as.vector(t(MeshDat)) #transpose and convert to vector ready for final matrix completion
  MeshDat <- matrix(MeshDat, length(MeshDat)/9,9, byrow = T) #create the final MeshDat matrix,  where each row is a triangle with three sets of XYZ coordinates for the triangles points
  return(list(MeshDat, XYZDat)) #return both the MeshDat and XYZDat as a list
}
